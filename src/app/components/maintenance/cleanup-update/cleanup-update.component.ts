...
@Component({
  selector: 'app-cleanup-update',
  templateUrl: './cleanup-update.component.html',
  styleUrls: ['./cleanup-update.component.scss']
})
export class CleanupUpdateComponent implements OnInit {

  updateCleanupForm: FormGroup;
  cleanup: Cleanup;
  tasks = [];
  ...
  ngOnInit() {
    ...
    this._cleanupService.getCleanup(this._id).subscribe(cleanup => {
      ...
      this.updateCleanupForm.patchValue({
        date: cleanup.date,
        place: cleanup.place
      });
  ...
  updateCleanup(): void {
    if (this.updateCleanupForm.invalid) {
      return;
    }
    this.cleanup = {
      _id: this._id,
      place: this.updateCleanupForm.value.place,
      date: this.updateCleanupForm.value.date,
      tasks: this.tasks
    };
    this._cleanupService.updateCleanup(this.cleanup._id, this.cleanup).subscribe(() => {
      this.goBack();
    }, error => {
      console.log(error);
    });
  ...
  deleteCleanup(): void {
    this._cleanupService.removeCleanup(this._id).subscribe(response => {
      this.goBack();
    }, error => {
      console.log(error);
    });
